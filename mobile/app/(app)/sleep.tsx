 ...
 *
 * Reads two Supabase tables (sibling agent owns the schema + the Samsung
 * Health write path):
 *
 ...
 *
 * Empty states: gracefully degrades for users with no Samsung Health
 * connection (CTA to Connect tab) and for users with partial data
 * (per-dimension "—" with helper text when not enough nights).
 */
...
import {
  Moon, Clock, Activity, BedDouble, Repeat, ArrowRight,
...
// ── DB row shapes ────────────────────────────────────────────────────────────
// Optional/nullable across the board because Samsung Health doesn't always
// report every stage (some sessions are duration-only). We treat missing
// stage data as "can't compute quality" rather than crashing.
...
export default function SleepPage() {
  ...
  // opened. We don't kick a Samsung sync here; the Heart page or the
  // Connect tab handles that and the data lands in the same DB.
  ...
  const durationDim: DimensionResult = useMemo(() => {
    if (sessions7.length === 0) {
      return {
        ...
        action:   'Wear your watch overnight and tap Sync — your duration shows up here once a session lands.',
        spark:    [],
      }
    }
  ...
  const qualityDim: DimensionResult = useMemo(() => {
    if (sessions7.length === 0) {
      return {
        ...
        action:   'Quality numbers appear once we have at least one full night of stage data from your watch.',
        spark:    [],
      }
    }
    ...
    if (stageBearing === 0 || totalSum === 0) {
      return {
        ...
        target:   null,
        headline: 'Deep ≥18%  ·  REM ≥20%',
        action:   'Your watch hasn’t reported sleep stages yet. Make sure stage tracking is enabled in Samsung Health → Settings → Sleep.',
        spark:    [],
      }
    }
  ...
  return (
    ...
    >
      ...
      {/* ── Empty state — no sleep data anywhere ─────────────────────────── */}
      {!loading && !hasAnyData && (
        <AnimateRise delay={0} style={s.card}>
          <Moon size={20} color={palette.indigo[400]} />
          <Text style={s.emptyTitle}>No sleep data yet</Text>
...
const s = StyleSheet.create({
  ...
  primaryBtnText:{ color: colors.primaryForeground, fontWeight: '700', fontSize: 13 },
