...
- **Assistant must auto-reload after every JS/TS edit.** The user does NOT want to be asked "shake to reload?" or "let me know when you've reloaded" — the assistant pushes the reload itself, every single time, by running:
  ...
  **Stale-Metro-cache scar (LOCKED, June 6 2026).** A force-stop + relaunch gives a fresh JS *context*, but it re-fetches whatever bundle **Metro currently has cached** — it does NOT force Metro to re-transform changed source. This repo lives under **OneDrive**, whose file-watcher frequently misses edits, so Metro can keep serving the OLD transform of a file you just edited. Symptom: you edit + force-reload (PID even changes), but the device still renders the OLD behaviour, indefinitely, across many reloads. **The June 6 2026 Pull-Up chart fix burned a full round of reloads on this** — the fix (chart switches to "Est. 1RM over time" when a bodyweight tier has loaded efforts) was confirmed on disk in the main repo AND Metro was confirmed running from the main repo, yet the phone kept showing the old "Max attempts over time" reps chart. Root cause: stale Metro transform cache. **Diagnosis:** confirm your edit is on disk (`grep` the main-repo file), confirm Metro's project root is the main repo (not a `.claude/worktrees` copy — check the Metro process command line), then if the device STILL shows old code, the cache is stale. **Fix:** kill Metro (`Stop-Process` the node PID bound to 8081) and restart it with **`--clear`** (`npx expo start --dev-client --clear --port 8081` from `C:\Users\motaz\OneDrive\Desktop\MyRX\mobile`). Metro prints `Bundler cache is empty, rebuilding` — that's the confirmation the cache was wiped. Then force-reload the app (or dev-menu Reload) to fetch the freshly-transformed bundle. A plain force-reload alone will NOT fix a stale cache; only `--clear` does. Don't tell the user to delete + re-log their data — the data is never the problem here.
