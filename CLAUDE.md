...
2. **"Train by percentage" card:**
   ...
   - **Hero card** (blue chrome) for the selected tile: big TickerNumber weight + `LABEL · % · reps` sub-line + a **prose coaching cue** rendered through the shared `CueText` component (the one locked cue format). The cue spells out a **warm-up ramp + the working set + a bar-speed reminder**. The ramp comes from `buildOlympicCue` → `olympicRamp`: two loadable jumps at ~60% & ~80% of the working weight, kept strictly between the empty bar and the work set (0-2 rungs, collapses on light loads), written INTO the sentence — the user chose prose over a chip row (T088 round-2 #2). Examples: Build → *"Warm up from the empty bar through 115 and 150 before 1-2 reps at 190 lb, around 85% of your best. Crisp singles and doubles, stopping the moment the bar slows."*; Peak → *"…before a heavy single at 230 lb, a new PR. Make or miss, never grind it out, speed is the signal."* CueText emphasizes the working weight blue and the ramp jumps foreground, so the work set reads as the standout number. Reps stay 1-3 BY DESIGN — there is deliberately NO high-rep option (technique + bar speed collapse past ~3 reps), and the cue reinforces it.
