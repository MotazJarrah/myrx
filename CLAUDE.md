...
- **VERIFY EVERY UPDATE AGAINST REAL DATA — "it shipped" ≠ "it works" (MANDATORY, locked 2026-06-08).** A green build and a matching deploy hash only prove code shipped; they say NOTHING about whether the feature is correct. Before calling ANY update done — every update, not just mirrors — prove it produces the right result. This was locked after a run of settings bugs (T109/T110) where the web admin "mirror" of a client's settings silently showed DEFAULTS — wrong fluid unit, wrong body-fat band, a missing custom meal slot, privacy toggles stuck ON — because the admin read path (the `get_user_for_admin` SECURITY DEFINER RPC) had a hand-written column list that never received the newer columns, and the build only ever confirmed the component READ `profile.X`, never that `profile.X` was actually populated. The whole failure class is "looked fine on defaults, wrong on real data," and the user kept being the one to catch it. Mandatory checks for every data-driven change:
  1. **Trace the read path end-to-end.** For each field the UI shows or edits, follow it back to its SOURCE — the `.select(...)` string, the RPC's `RETURNS TABLE` column list, the function body — and confirm that source actually returns that field. `obj.field` in a component proves nothing if `obj` came from a query/RPC that omits `field` (it'll just render the default). COROLLARY: when mobile gains a new `profiles` column, EVERY admin/coach read path that feeds a web surface must be extended too — especially `get_user_for_admin` and any other SECURITY DEFINER RPC with a hand-written column list — or the web silently falls back to defaults.
  2. **Test with REAL, NON-DEFAULT data — never defaults.** A field rendering its default value "looks correct" and hides whether the real value actually flows. Use the Supabase MCP to read a specific account whose values are deliberately NON-default (metric units, a non-`average` body-fat band, a custom meal slot, a privacy toggle turned off, etc.), then confirm the surface shows THOSE values. If the rendered UI isn't reachable from here, cross-check: read the account's actual stored DB values and confirm the read path + component logic would render exactly those, field by field.
  3. **For anything called a "mirror," diff the LIVE source control-by-control.** Open the real source (the live mobile component, not the spec, not memory) and compare every control's label, sub-label, option set + values, section placement, AND the data each control reads — one by one. `mi` vs `mi · yd`, `Ounces` vs `Fluid ounces`, `≤12%` vs `≤14% BF`, a missing description line — that surface-level drift is exactly what gets caught later. Approximating labels from memory is how the drift happens.
  4. **Observe the result; don't assume it.** Use the tools you have: query the DB (Supabase MCP) for the ground truth, inspect the rendered output wherever the surface is reachable, and only ask the user to confirm the parts you genuinely cannot observe (e.g. a mobile-device-only flow). "Deployed, please verify" is NOT verification — do as much of it yourself as the tools allow, and when a check is impossible from here, say so explicitly and name the exact thing for the user to look at instead of silently skipping it.
...
### RPC functions
- `get_users_for_admin()` — returns all client profiles. Actual columns (verified Jun 9 2026): id, full_name, email, gender, birthdate, current_weight, weight_unit, current_height, height_unit, created_at, has_plan, avatar_url, deactivated_at, is_self_coached, last_seen_at, share_online_status, share_last_seen, scheduled_for_deletion_at, anonymized_at. (Returns `has_plan` + the body-comp fields needed for plan/TDEE math; does NOT return `is_superuser`.)
