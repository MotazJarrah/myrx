...
- **NEVER CHANGE AN APPROVED/GIVEN DIRECTION WITHOUT COMING BACK FIRST (MANDATORY — locked Jun 13 2026, the user was emphatic).** When the user gives a direction — a specific fix, approach, or mechanism (e.g. "fix the DNS") — execute EXACTLY that. The moment it turns out to be blocked, infeasible, costlier than expected, OR you think a different approach would be better, STOP and come back for an explicit go on the NEW direction BEFORE doing anything else. NEVER silently substitute a lesser/secondary mechanism, a bandaid, a workaround, a fallback, or a scope change — and never "downgrade" the chosen fix to an easier one. A blocker is a reason to ASK, never a license to improvise, even when you are 100% sure the alternative is fine. The user owns the direction; you execute it or you come back. The ONLY thing you may do unprompted when blocked is gather read-only info to frame the question. No exceptions. (Triggered Jun 13 2026: user said "fix the DNS" for the www 522 → assistant downgraded to a redirect rule; earlier it logged a bandaid + offered options instead of fixing, and deleted more accounts than the user's stated cleanup — all the same failure of substituting the assistant's decision for the user's direction.)
