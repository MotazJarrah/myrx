...
TASKS = [
    ...
    ("T098", "Coach-lapse downgrade: make resolveTier active-sub-aware (clients + coach-self)", "Billing", "Cross", "Open",
     "From T096 Step-A verification: resolveTier (mobile dashboard.tsx + web CoachClientDetail/AdminUserDetail) comps coached clients + coaches to FullRX off the LINK (coach_id / is_coach), NOT the coach's ACTIVE subscription. The active-sub RPCs client_has_active_coach() / is_active_coach() EXIST but resolveTier doesn't use them -- so a client of a LAPSED coach (and a coach whose own sub lapsed) stays FullRX. Launch-readiness correctness gap.",
     "OPEN -- before launch: make the entitlement active-sub-aware (resolveTier consults client_has_active_coach()/is_active_coach(), OR a lapse job downgrades coach_id-linked clients + coach-self) and tie into the CLAUDE.md 'pick a plan' lapse flow. NOT blocking pre-launch (everyone trial/active).",
     "mobile dashboard.tsx resolveTier; web resolveTier (CoachClientDetail/AdminUserDetail); client_has_active_coach()/is_active_coach() RPCs",
     "2026-06-07"),
    ...
    ("T100", "Efforts (AdminUserActivity) -- T086 athlete-education strip pass", "Coach/Admin", "Web", "Open",
