...
TASKS = [
    ...
    ("T093", "Coach-units pass -- coach sees client data in the coach's units", "Coach/Admin", "Web coach", "In progress",
     ...
     "REMAINING: (1) strength coach DETAIL pages (AdminStrengthWeighted/RepsOnly/Assisted/Bodyweight/Carry/Load) -- this is MORE than a source-swap (found 2026-06-06): the display unit is `unitLock || best.unit || profileUnit` (the BEST EFFORT's LOGGED unit, e.g. Weighted L400), and the 1RM + rep-max projections compute IN that unit. So coach-units needs PER PAGE: (a) change precedence to `unitLock || coachUnit` (coach via useAuth), (b) CONVERT the best 1RM + all displayed weights + the efforts list from the logged unit to the coach unit -- NO kg<->lb convert helper exists in these files yet, add one; (c) KEEP the strongman kg unitLock and KEEP the client's current_weight as the e1RM/effective-load MATH input (convert for display only). AdminStrengthLeverage (time-only) + Family (wrapper) need no change. Do this as a focused chunk, one page as template then fan out + build-verify. (2) Calories + Hydration coach tabs -- Hydration fluid oz/mL -> coach fluid_unit; Calories kcal+g no-convert but check any plan bodyweight display; Sleep is hours -> no convert. Then flip T093 to Done.",
     "DONE: web AdminUserBody.jsx + AdminCardio{Pace,BeatYourBest,Swimming}Detail.jsx (ae4fe28, 276181b, live index-CSE_g1K8). PENDING: web admin/detail/AdminStrength{Weighted,RepsOnly,Assisted,Bodyweight,Carry,Load}Detail.jsx; Calories + Hydration coach tabs",
     "2026-06-06"),
]
