...
TASKS = [
    ...
    ("T093", "Coach-units pass -- coach sees client data in the coach's units", "Coach/Admin", "Web coach", "In progress",
     "Earlier this session the user set a units model (never captured -- capture-first miss): the coach/admin views a client's values in the COACH's units (not the client's), add-form unit toggles default to the coach's units, and a mixed-unit rule -- record LISTS keep each effort's own logged unit while GRAPHS + headline numbers convert to the viewer (coach) unit; by-design locks still win (strongman kg, rucking mi, Concept2 ergs metric). Decision noted: 'option 2 -- do it all in one pass'. CONFLICT TO RESOLVE: T079's note says the coach Bodyweight tab was intentionally built in the CLIENT's units; this pass reverses that. DONE so far: coach Bodyweight tab flipped to coach units -- chart + Current/BMI/Ideal/Trend stats + Add-weigh-in default now read the logged-in coach's weight_unit via useAuth, while the weigh-in LIST keeps each entry's logged unit (committed ae4fe28, deployed, live).",
     "BLOCKER -- confirm direction with the user: keep+extend coach-units, or revert Bodyweight to client-units. If coach-units stands, remaining surfaces: the cardio + strength coach DETAIL pages (they currently self-fetch + show the CLIENT's distance/weight unit -- verified AdminCardioPaceDetail L634) need the coach's unit for graphs/headline + logged unit for record lists; plus any unit displays on the Calories / Sleep / Hydration coach tabs. Efforts (T080) is already done; Heart is bpm (no unit).",
     "web AdminUserBody.jsx (done, ae4fe28); pending: web admin/detail/AdminCardio*Detail + AdminStrength*Detail, AdminUserDetail.jsx, Calories/Sleep/Hydration coach tabs",
     "2026-06-06"),
]
