 ...
 *
 * Two tabs: Messages and Suggestions, each with an unread badge.
...
function MessagesTab({ users, messages, onMarkRead, onNewMessage, onDeleteMessage }) {
  ...
  // Build conversation list from non-suggestion messages, scoped to roster users
  ...
  // Two empty states:
  //   - Roster has no clients OR no conversations yet → coaching-voice empty state
  //   - Otherwise → normal split view
  if (conversations.length === 0) {
    const hasClients = users.length > 0
    return (
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-3">
              ...
              {conversation.map(msg => (
                <div key={msg.id} className={`flex py-0.5 ${msg.from_admin ? 'justify-end' : 'justify-start'}`}>
                  {msg.from_admin ? (
                    /* Coach bubble: swipe left to reveal delete — overflow-hidden clips red zone to rounded shape */
                    <SwipeDelete
                      swipe
  ...
  if (suggestions.length === 0) {
    return (
      <div className="rounded-xl border border-border bg-card py-16 px-6 text-center">
        ...
        <p className="text-sm text-muted-foreground">No suggestions yet from your clients.</p>
        <p className="text-xs text-muted-foreground/70 mt-2 max-w-md mx-auto leading-relaxed">
          Suggestions are how clients flag things they want changed — a movement substitution,
          a macro tweak, a schedule shift. When they submit one, it lands here so you can act
          on it the next time you update their plan.
        </p>
      </div>
    )
  }
...
export default function CoachMessages() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      // Same filter pattern as CoachClients.jsx.
    ...
    const channel = supabase
      ...
      // Roster realtime — when a new client joins or one is deactivated, refetch
      // so the conversation list reflects the new shape. Cheap and avoids the
      // "I just invited a client but their messages don't appear" case.
      .on('postgres_changes', {
        event:  'INSERT',
        schema: 'public',
        table:  'profiles',
      ...
      .on('postgres_changes', {
        ...
        setUsers(prev => {
          ...
          // Otherwise update in place (e.g. avatar / name change)
  ...
  // Always-fresh ref so the auto-mark effect never captures a stale messages closure
