...
import {
  ...
  CornerDownLeft, Calendar, ChevronDown, ShieldCheck,
...
export function SettingsTab({ profile, user }) {
  ...
  return (
    <form onSubmit={handleSave} className="space-y-5">
      ...
      {/* Meal layout — shared editor (also used by the admin client-settings
          mirror). Owns its own slot state + Save button. */}
      <div className="animate-rise rounded-2xl border border-border bg-card p-6 space-y-4" style={{ animationDelay: '60ms' }}>
        ...
        <MealLayoutEditor
          ...
          refreshOnSave
          note="Removing a custom slot only removes it from your default layout — past food entries logged under that slot are preserved and will still appear when you view those days."
        />
      </div>
