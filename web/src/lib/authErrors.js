 ...
 * message passes through unchanged — better to show a slightly
 * technical real message than to mislabel the actual failure.
 *
 * MIRROR — kept in sync with mobile/src/lib/authErrors.ts under the
 * cross-platform consistency rule. Update both when adding new codes.
 */

const SUPPORT_EMAIL = 'team@myrxfit.com'

// Supabase auth error codes → friendly messages. Lookup is by `code`
// first (most reliable — Supabase v2 always sets it) and falls back
...
// doesn't include a stable `code` field (older GoTrue versions, some
// rate-limit responses, generic 422s). Lowercase comparison.
const MESSAGE_FALLBACKS = [
  ...
  { test: 'expired',                  key: 'otp_expired' },  // last-resort for OTP flows
...
export function mapAuthError(error) {
  ...
  if (isNetworkError(error)) {
    return { ...error, code: 'network_error', message: NETWORK_MESSAGE }
  }
  const key = resolveKey(error)
  if (!key) return error
  return { ...error, code: key, message: CODE_MESSAGES[key] }
}
