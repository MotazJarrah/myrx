 ...
 *
 * Port of `mobile/src/lib/countries.ts`. Keep in sync — both surfaces
 * read the same Supabase columns, so the same set of supported countries
 ...
 * `flag` uses unicode regional-indicator emojis so we don't need an
 * image asset per country.
 */
 ...
 */
export function matchCountryFromPhone(phone) {
